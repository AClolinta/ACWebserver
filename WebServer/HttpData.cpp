...
/// @brief 提供一个简便的方法来获取文件扩展名对应的 MIME
/// 类型，如果不存在对应的扩展名，则返回默认的 MIME 类型。
/// @param suffix
/// @return
...
// Parser the request headers which consist by key and value
    ...
    for (; i < str.size() && notFinish; ++i) {
        switch (hState_) {
            ...
            case H_CR: {
                if (str[i] == '\n') {
                    hState_ = H_LF;
                    std::string key(str.begin() + key_start,
                                    str.begin() + key_end);
            ...
            case H_LF: {
                if (str[i] == '\r') {
                    // End of the paresr because there are two \r\n
                    hState_ = H_END_CR;
                } else {
                    key_start = i;
                    hState_ = H_KEY;
...
AnalysisState HttpData::analysisRequest() {
    if (method_ == METHOD_POST) {
        ...
        if (fileName_ == "favicon.ico") {
            ...
            header +=
                "Content-Length: " + std::to_string(sizeof favicon) + "\r\n";
            header += "Server: ACLOLINTA WEB Server\r\n";
            header += "\r\n";
            //
            outBuffer_ += header;
        ...
        // Open the file and mapping it to memory
...
void HttpData::handleError(int fd, int err_num, std::string short_msg) {
    ...
    body_buff += "<html><title>FATAL Error</title>";
    body_buff += "<body bgcolor=\"ffffff\">";
    body_buff += std::to_string(err_num) + short_msg;
    body_buff += "<hr><em> ACLOLINTA Web Server</em>\n</body></html>";
    //
